...
$qry = $conn->query("SELECT * FROM apdk where md5(id) = '{$_GET['id']}' ")->fetch_array();
foreach($qry as $k => $v){
	if($k == 'title')
		$k = 'ftitle';
	$$k = $v;
...
<script>
	...
	$('#share').click(function(){
		uni_modal("<i class='fa fa-share'></i> Share this document using the link.","modal_share_link_apdk.php?did=<?php echo md5($id) ?>")
	})
</script>